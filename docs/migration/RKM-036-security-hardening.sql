-- RKM-036: Security hardening
-- - Role-based admin permissions (user_roles + is_admin())
-- - Restrict resident data reads to authenticated users
-- - Remove email-string-based admin checks in RLS
...
-- Backfill: keep existing designated admin account working immediately.
insert into public.user_roles (user_id, role)
select id, 'admin'
from auth.users
where lower(coalesce(email, '')) = 'swanston@roomingkos.com'
on conflict (user_id) do update
set role = excluded.role, updated_at = timezone('utc'::text, now());
